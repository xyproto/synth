...
func (cfg *Settings) GenerateClap() ([]float64, error) {
	...
	for burst := 0; burst < burstCount; burst++ {
		...
		// Generate filtered noise for each burst
		burstNoise := GenerateNoise(cfg.NoiseType, burstSamples, cfg.NoiseAmount)
...
func (cfg *Settings) GenerateSnare() ([]float64, error) {
	...
	// Generate the noise part (snare "rattle") using filtered white or pink noise
...
func (cfg *Settings) GenerateTom() ([]float64, error) {
	...
	// Add a bit of noise to simulate drum head vibrations
	noiseSamples := GenerateNoise(cfg.NoiseType, numSamples, cfg.NoiseAmount)
